...
public class Line implements Comparable<Line>, Iterable<Point> {
    ...
    * @param p point to check to add
    * @return return if the point has been added
    */
   public boolean add(Point p) {
      if (line == null || line.size() == 0) {
         ...
         if (p.slopeOrder.compare(first(), last()) == 0) {
            Iterator<Point> itr = iterator();
            while (itr.hasNext()) {
               if (p.compareTo(itr.next()) == 0) {
                  return false;
               }
            }
            line.add(p);
    ...
    * @return return numbers of points in the line
    */
   public int length() {
      if (line == null) {
         return 0;
   ...
   public int compareTo(Line that) {
      if (this == null || this.length() == 0 || this.length() == 1) {
         if (that == null || that.length() == 0 || that.length() == 1) {
            return 0;
         }
         else {
            return -1;
      ...
      else if (that == null || that.length() == 0 || that.length() == 1) {
         return 1;
      }
      else {
